...
var Fixtures = map[string]struct {
	...
	"time-binary": {
		Encode: func(w *Writer) error { return w.WriteBinary(time.Unix(60, 0)) },
		Decode: func(r *Reader) (interface{}, error) {
			var out time.Time
			err := r.ReadBinary(&out)
			return out, err
		},
		Buffer: []byte{0xf, 0x1, 0x0, 0x0, 0x0, 0xe, 0x77, 0x91, 0xf7, 0x3c, 0x0, 0x0, 0x0, 0x0, 0x0, 0xf0},
		Value:  time.Unix(60, 0),
	},
	"time-text": {
		Encode: func(w *Writer) error { return w.WriteText(time.Unix(60, 0)) },
		Decode: func(r *Reader) (interface{}, error) {
			var out time.Time
			err := r.ReadText(&out)
			return out, err
		},
		Buffer: []byte{0x19, 0x31, 0x39, 0x37, 0x30, 0x2d, 0x30, 0x31, 0x2d, 0x30, 0x31, 0x54,
			0x30, 0x34, 0x3a, 0x30, 0x31, 0x3a, 0x30, 0x30, 0x2b, 0x30, 0x34, 0x3a, 0x30, 0x30},
		Value: time.Unix(60, 0),
	},
}
